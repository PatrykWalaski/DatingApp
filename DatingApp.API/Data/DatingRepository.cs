...
namespace DatingApp.API.Data
{
    public class DatingRepository : IDatingRepository
    {
        ...
        {
            // checking if logged user already liked another one so we dont like the same person few times
            // if this like doesnt exist its gonna return null (FirstOrDefault - default is null)
        ...
        public async Task<PagedList<User>> GetUsers(UserParams userParams)
        {
            ...
            if(userParams.MinAge != 18 || userParams.MaxAge != 99)
            {
                ...
                users = users.Where(u => u.Birth >=  minDateOfBirth && u.Birth <= maxDateOfBirth);
            }
        ...
        private async Task<IEnumerable<int>> GetUserLikes(int id, bool likers)
        {
            ...
            // if true we want to return all of the users that currently liked our logged in user
            if(likers)
            {
        ...
        {
            return await _context.SaveChangesAsync() > 0;  //the number means number of changes saved in database, if 0 no changes so returns false
        }
        ...
        {
            var messages = await _context.Messages.Include(s => s.Sender).ThenInclude(p => p.Photos)
                                           .Include(r => r.Recipient).ThenInclude(p => p.Photos)
